...
class Drawer():
    ...
    def update_plot(self, frame):
        ...
        if self.dia_1 == True:
            ...
            theta = 2*np.pi/6*np.sin(2*np.pi*FREQUENCY*(t)) + np.pi/2 
            x_c = self.circular_motions_radius*np.cos(-theta)
            y_c = self.circular_motions_radius/2 + self.circular_motions_radius*np.sin(-theta)
    ...
    def initilize_plot(self):
        ...
        if self.dia_1 == True:
            ...
            self.half_circle = Arc((25,22.5+self.circular_motions_radius/2), 2*self.circular_motions_radius, 2*self.circular_motions_radius, theta1=210, theta2=-30, color='k', fill=False)
            ...
            self.horizontal_line_center = plt.Line2D([22.5,27.5],[22.5+self.circular_motions_radius/2,22.5+self.circular_motions_radius/2], color='k', linewidth=4)
            ...
            self.boundary_line_right = plt.Line2D([25,25+15*np.cos(np.pi/6)],[22.5+0,22.5+15*np.sin(np.pi/6)],color='c', linewidth=3)
            self.boundary_line_left = plt.Line2D([25,25-15*np.cos(np.pi/6)],[22.5+0,22.5+15*np.sin(np.pi/6)],color='c', linewidth=3)
            self.ax.add_line(self.horizontal_line)
            self.ax.add_line(self.vertical_line)  
            self.ax.add_patch(self.guide_circle)
            self.ax.add_line(self.guide_handle)
...
if __name__ == '__main__':
    ...
    drawer = Drawer(guide_direction='horizontal',u_shaped_maneuver=True)
